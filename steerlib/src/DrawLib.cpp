...
// drawStar()
//
void DrawLib::drawMyAgent(const Point & pos, const Vector & dir, float radius, const Color& color)
{
	Point newPos = pos;
	for (int i = 0; i < 100; i++){
		...
		// draw star
		drawQuad(newPos - side, newPos - forward, newPos + side, newPos + forward);
//		drawQuad(newPos - rotateInXZPlane(side, theta), newPos - rotateInXZPlane(forward, theta), newPos + rotateInXZPlane(side, theta), newPos + rotateInXZPlane(forward, theta));
	}
}
